...
class m_petugaspendaftaran extends CI_Model {
	...
	public function selectjoin($table,$table2,$join,$table3,$join3){
		$this->db->select('*')->from($table)->join($table2,$join)->join($table3,$join3);
        return $this->db->get();
	}
	...
	public function canceldaftar($where){
    	$this->db->where($where);
		$this->db->delete('t_berobat');
    }
